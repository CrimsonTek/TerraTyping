...
public static class Calc
{
        ...
        where D : Wrapper, ITarget, IDefensiveElements, IAbility
    {
        ...
        for (int i = 0; i < defensiveArr.Length; i++)
        {
            for (int j = 0; j < offensiveArr.Length; j++)
            {
                ...
                knockback *= postAttackerModifierEffectiveness;
            }
        }
        knockback -= 1;
        knockback *= ServerConfig.Instance.KnockbackScaling;
        knockback += 1;
    ...
    {
        float stabAmount = ServerConfig.Instance.STAB;
        ...
        {
            float dimishingReturnScalar = ServerConfig.Instance.STABDiminishingReturnScalar;
    ...
    public static void OtherModifyHitBy(OtherWrapper other, PlayerWrapper player, ref Player.HurtModifiers modifiers)
    {
        ModifyHitBy(other, player, ref modifiers.FinalDamage, ref modifiers.Knockback);
    }

    public static void ModifyHitBy<A, D>(A attacker, D target, ref StatModifier finalDamage, ref StatModifier knockback)
        where A : Wrapper, IOffensiveType, IAbility, IDamageClass, IStatsBuffed
        where D : Wrapper, ITarget, IDefensiveElements, IAbility
    {
        DamageReturn damageReturn = Damage(attacker, target);
        ...
        knockback *= damageReturn.knockback;
    ...
    /// <summary>
    /// Handles abilities that buff on hit and 
    /// </summary>
    public static void OnHit<A, D>(A attacker, D defender)
        where A : Wrapper, IOffensiveType, IAbility, IDamageClass, IHitbox, ITeam, IStatsBuffed
        where D : Wrapper, ITarget, IDefensiveElements, IAbility, ITeam
    {
        // rewrite this check with Player.ProcessHitAgainstNPC
        if (attacker.Hitbox.Intersects(defender.GetRect()) && defender.Active && attacker.GetTeam() != defender.GetTeam())
        {
            ...
            if (defender.GetCombatTextCooldown() <= 0)
            {
                ...
                Message messageHitEnemy = GetAbility(attacker.GetAbility).MessageHitEnemy(new MessageHitEnemyParameters(attacker.OffensiveElements, defender)).message;
