...
public class PrintCSVCommand : ModCommand
{
    ...
    private static void PrintVanilla(CommandCaller caller, PrintType printType)
    {
        ...
        {
            case PrintType.NPC:
                PrintAllVanilla<NPC>(caller,
                    NPCID.NegativeIDCount + 1,
                    NPCID.Count,
                    typeof(NPCID),
                    "npcTypesOverride",
                    (n) => n.TypeName,
                    (n, type) => n.SetDefaults(type),
                    (npc) => true);
                break;
    ...
    static string ExtraWeaponColumns(Item item)
    {
        ...
        return $",{toolType},{item.OriginalRarity},{item.DamageType.DisplayName},{item.OriginalDamage},{item.value}";
    }
    ...
    private static void PrintAllFromMod<T>(
        ...
        string fileName,
        Func<T, LocalizedText> getDisplayName,
    ...
    where T : ModType
    {
        ...
        foreach (T tObj in mod.GetContent<T>())
        {
            if (select is null || select(tObj))
            {
                ...
                streamWriter.WriteLine($"{internalName},{getDisplayName(tObj).Value},{extra}");
                streamWriter.Flush();
            }
        }
    ...
    private static void PrintArmorVanity(CommandCaller caller, bool armor, bool vanity)
    {
        ...
        PrintAllVanilla<Item>(caller, 1, ItemID.Count, typeof(ItemID),
            ...
            (item) => (item.legSlot is not -1 || item.bodySlot is not -1 || item.headSlot is not -1) && (item.OriginalDefense > 0 || item.type == ItemID.WoodGreaves ? armor : vanity),
