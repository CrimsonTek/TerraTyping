...
using TerraTyping.Content.Accessories.HeldItems;
...
public class ItemDamageBoost : GlobalItem
{
    ...
    {
        AbilityFlyweight ability = player.GetModPlayer<PlayerTyping>().GetAbility();
    ...
    public override void ModifyTooltips(Item item, List<TooltipLine> tooltips)
    {
        ...
        void ModifyTooltip(Boost[] boosts, string name)
        {
            for (int i = 0; i < boosts.Length; i++)
            {
                ...
                bool bonus = (float)boosts[i].Multiplier > 1;
                string key = bonus ? "Mods.TerraTyping.Tooltip.Bonus" : "Mods.TerraTyping.Tooltip.Penalty";
                double rounded = Math.Round(((float)boosts[i].Multiplier - 1) * 100);
                TooltipLine line = new TooltipLine(Mod, name, Language.GetText(key).Format(boosts[i].reason, rounded));
                tooltips.Add(line);
            }
        }
    }
